      ...
      <div class="md-layout-item md-medium-size-100 md-xsmall-size-100 md-size-100">
        <md-card>
          <md-card-header data-background-color="green">
            <h4 class="title">Liste des Commandes des Promos</h4>
          </md-card-header>
          <md-card-content>
            <div>
              <md-table table-header-color="green">
                <md-table-row slot="md-table-row" v-for="com in commandes" v-bind:key="com.Id">
                  <md-table-cell md-label="Nom">Num: 0{{ com.Numero_client }}</md-table-cell>
                  <md-table-cell md-label="adresse">Adresse livraison : {{ com.adresse }}</md-table-cell>
...
export default {
  ...
  mounted() {
    api.get("/commande").then(res => {
      this.commandes = [...res.data.results];
    });
  },
  methods: {
    // modifier(p) {
